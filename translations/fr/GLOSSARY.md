...
| product memory | mémoire produit | Mémoire durable et riche : sources, recherches, décisions, alternatives, hypothèses et contexte. |
| product memory workspace | espace de mémoire produit | Outil ou corpus qui porte la mémoire ; Notion est un exemple, pas une obligation. |
| engineering repository | dépôt d'ingénierie | Dépôt versionné contenant constitution active, provenance, pilotage, code, tests et preuves. |
| semantic compilation | compilation sémantique | Transformation fidèle et traçable de la mémoire active en contrat d'ingénierie ; ne signifie pas export brut. |
| Ready to Compile | Prêt à compiler | Gate indiquant que la mémoire peut être compilée sans invention ni réduction silencieuse. |
| Ready to Develop | Prêt à développer | Gate indiquant que le dépôt peut être repris sans rouvrir la mémoire d'origine. |
| compilation manifest | manifeste de compilation | Artefact reliant sources, sorties, omissions, redactions, versions de canons et baseline Git. |
| product baseline | baseline produit | Commit ou tag identifiant la constitution active remise à l'agent de développement. |
| development-agent takeover | reprise par l'agent de développement | Bascule vers le Git opérationnel, le pilotage et l'implémentation dans le dépôt. |
