# Statut de traduction française

L'anglais reste la source canonique. Ce tableau suit l'état de la traduction française maintenue officiellement.

Statuts autorisés :

- `translated` : traduction française présente et alignée avec la source connue.
- `partial` : traduction présente mais volontairement incomplète.
- `missing` : traduction absente.
- `fr-sync-needed` : la source anglaise a changé ou doit être revue côté français.

| English source file | French translated file | Status | Notes |
| --- | --- | --- | --- |
| `README.md` | `translations/fr/README.md` | translated | Entrypoint français officiel. |
| `docs/00-overview.md` | `translations/fr/docs/00-overview.md` | translated |  |
| `docs/01-manifesto.md` | `translations/fr/docs/01-manifesto.md` | translated |  |
| `docs/02-core-principles.md` | `translations/fr/docs/02-core-principles.md` | translated |  |
| `docs/03-premium-standard.md` | `translations/fr/docs/03-premium-standard.md` | translated |  |
| `docs/04-product-discovery.md` | `translations/fr/docs/04-product-discovery.md` | translated |  |
| `docs/05-deductive-product-profiling.md` | `translations/fr/docs/05-deductive-product-profiling.md` | translated |  |
| `docs/06-challenge-gate.md` | `translations/fr/docs/06-challenge-gate.md` | translated |  |
| `docs/07-product-to-engineering-layers.md` | `translations/fr/docs/07-product-to-engineering-layers.md` | translated |  |
| `docs/08-dual-ui-model.md` | `translations/fr/docs/08-dual-ui-model.md` | translated |  |
| `docs/09-context-and-capability-mesh.md` | `translations/fr/docs/09-context-and-capability-mesh.md` | translated |  |
| `docs/10-capability-graph.md` | `translations/fr/docs/10-capability-graph.md` | translated |  |
| `docs/11-event-and-artifact-model.md` | `translations/fr/docs/11-event-and-artifact-model.md` | translated |  |
| `docs/12-cost-and-resource-governance.md` | `translations/fr/docs/12-cost-and-resource-governance.md` | translated |  |
| `docs/13-autonomous-worker-runtime.md` | `translations/fr/docs/13-autonomous-worker-runtime.md` | translated |  |
| `docs/14-quality-gates.md` | `translations/fr/docs/14-quality-gates.md` | translated |  |
| `docs/15-feedback-integration.md` | `translations/fr/docs/15-feedback-integration.md` | translated |  |
| `docs/16-safety-and-risk-model.md` | `translations/fr/docs/16-safety-and-risk-model.md` | translated |  |
| `docs/17-testing-and-sandboxing.md` | `translations/fr/docs/17-testing-and-sandboxing.md` | translated |  |
| `docs/18-adapters-overview.md` | `translations/fr/docs/18-adapters-overview.md` | translated |  |
| `docs/19-public-method-limitations.md` | `translations/fr/docs/19-public-method-limitations.md` | translated |  |
| `docs/20-autonomous-vision-to-product.md` | `translations/fr/docs/20-autonomous-vision-to-product.md` | translated | Extension canonique Vision-to-Product v0.2. |
| `templates/product-brief-template.md` | `translations/fr/templates/product-brief-template.md` | translated |  |
| `templates/product-profiling-template.md` | `translations/fr/templates/product-profiling-template.md` | translated |  |
| `templates/north-star-template.md` | `translations/fr/templates/north-star-template.md` | translated |  |
| `templates/vision-template.md` | `translations/fr/templates/vision-template.md` | translated |  |
| `templates/challenge-report-template.md` | `translations/fr/templates/challenge-report-template.md` | translated |  |
| `templates/domain-map-template.md` | `translations/fr/templates/domain-map-template.md` | translated |  |
| `templates/capability-map-template.md` | `translations/fr/templates/capability-map-template.md` | translated |  |
| `templates/dual-ui-model-template.md` | `translations/fr/templates/dual-ui-model-template.md` | translated |  |
| `templates/capability-spec-template.md` | `translations/fr/templates/capability-spec-template.md` | translated |  |
| `templates/worker-mission-charter-template.md` | `translations/fr/templates/worker-mission-charter-template.md` | translated |  |
| `templates/quality-gates-template.md` | `translations/fr/templates/quality-gates-template.md` | translated |  |
| `templates/autonomy-run-report-template.md` | `translations/fr/templates/autonomy-run-report-template.md` | translated |  |
| `templates/feedback-intake-template.md` | `translations/fr/templates/feedback-intake-template.md` | translated |  |
| `templates/vision-to-product-control-pack-template.md` | `translations/fr/templates/vision-to-product-control-pack-template.md` | translated | Control Pack compact et adaptable. |
| `prompts/README.md` | `translations/fr/prompts/README.md` | translated |  |
| `prompts/system-prompts/autonomous-product-architect.md` | `translations/fr/prompts/system-prompts/autonomous-product-architect.md` | translated |  |
| `prompts/system-prompts/deductive-product-profiler.md` | `translations/fr/prompts/system-prompts/deductive-product-profiler.md` | translated |  |
| `prompts/system-prompts/challenge-agent.md` | `translations/fr/prompts/system-prompts/challenge-agent.md` | translated |  |
| `prompts/system-prompts/ux-ui-premium-designer.md` | `translations/fr/prompts/system-prompts/ux-ui-premium-designer.md` | translated |  |
| `prompts/system-prompts/autonomous-worker-supervisor.md` | `translations/fr/prompts/system-prompts/autonomous-worker-supervisor.md` | translated |  |
| `prompts/system-prompts/documentation-architect.md` | `translations/fr/prompts/system-prompts/documentation-architect.md` | translated |  |
| `prompts/system-prompts/vision-to-product-orchestrator.md` | `translations/fr/prompts/system-prompts/vision-to-product-orchestrator.md` | translated | Orchestration de bout en bout. |
| `prompts/starter-prompts/start-new-product-method.md` | `translations/fr/prompts/starter-prompts/start-new-product-method.md` | translated |  |
| `prompts/starter-prompts/bootstrap-existing-project.md` | `translations/fr/prompts/starter-prompts/bootstrap-existing-project.md` | translated |  |
| `prompts/starter-prompts/convert-idea-to-product-layers.md` | `translations/fr/prompts/starter-prompts/convert-idea-to-product-layers.md` | translated |  |
| `prompts/starter-prompts/prepare-codex-build-plan.md` | `translations/fr/prompts/starter-prompts/prepare-codex-build-plan.md` | translated |  |
| `prompts/starter-prompts/prepare-hermes-worker-run.md` | `translations/fr/prompts/starter-prompts/prepare-hermes-worker-run.md` | translated |  |
| `prompts/starter-prompts/model-dual-ui.md` | `translations/fr/prompts/starter-prompts/model-dual-ui.md` | translated | Added French translation for helper prompt. |
| `prompts/starter-prompts/create-capability-map.md` | `translations/fr/prompts/starter-prompts/create-capability-map.md` | translated | Added French translation for helper prompt. |
| `prompts/starter-prompts/create-worker-mission-charter.md` | `translations/fr/prompts/starter-prompts/create-worker-mission-charter.md` | translated | Added French translation for helper prompt. |
| `prompts/review-prompts/audit-method-quality.md` | `translations/fr/prompts/review-prompts/audit-method-quality.md` | translated |  |
| `prompts/review-prompts/audit-safety-risks.md` | `translations/fr/prompts/review-prompts/audit-safety-risks.md` | translated |  |
| `prompts/review-prompts/audit-ui-clarity.md` | `translations/fr/prompts/review-prompts/audit-ui-clarity.md` | translated |  |
| `prompts/review-prompts/audit-transversality.md` | `translations/fr/prompts/review-prompts/audit-transversality.md` | translated |  |
| `prompts/review-prompts/audit-costs-and-resources.md` | `translations/fr/prompts/review-prompts/audit-costs-and-resources.md` | translated |  |
| `runbooks/setup-codex.md` | `translations/fr/runbooks/setup-codex.md` | translated |  |
| `runbooks/setup-hermes.md` | `translations/fr/runbooks/setup-hermes.md` | translated |  |
| `runbooks/setup-generic-cli-agent.md` | `translations/fr/runbooks/setup-generic-cli-agent.md` | translated |  |
| `runbooks/run-autonomous-worker.md` | `translations/fr/runbooks/run-autonomous-worker.md` | translated |  |
| `runbooks/safe-github-workflow.md` | `translations/fr/runbooks/safe-github-workflow.md` | translated |  |
| `runbooks/telegram-reporting-pattern.md` | `translations/fr/runbooks/telegram-reporting-pattern.md` | translated |  |
| `runbooks/quota-aware-execution.md` | `translations/fr/runbooks/quota-aware-execution.md` | translated |  |
| `adapters/codex.md` | `translations/fr/adapters/codex.md` | translated |  |
| `adapters/hermes.md` | `translations/fr/adapters/hermes.md` | translated |  |
| `adapters/generic-chatbot.md` | `translations/fr/adapters/generic-chatbot.md` | translated |  |
| `adapters/generic-cli-agent.md` | `translations/fr/adapters/generic-cli-agent.md` | translated |  |
| `examples/README.md` | `translations/fr/examples/README.md` | translated |  |
| `examples/example-product-intake.md` | `translations/fr/examples/example-product-intake.md` | translated |  |
| `examples/example-challenge-report.md` | `translations/fr/examples/example-challenge-report.md` | translated |  |
| `examples/example-capability-graph.md` | `translations/fr/examples/example-capability-graph.md` | translated |  |
| `examples/example-dual-ui-model.md` | `translations/fr/examples/example-dual-ui-model.md` | translated |  |
| `examples/example-worker-mission.md` | `translations/fr/examples/example-worker-mission.md` | translated |  |
| `examples/example-vision-to-product.md` | `translations/fr/examples/example-vision-to-product.md` | translated | Exemple canonique synthétique complet. |
| `case-studies/hermes-agency-os/README.md` | `translations/fr/case-studies/hermes-agency-os/README.md` | translated | Sanitized case study preserved. |
| `case-studies/hermes-agency-os/lessons-learned.md` | `translations/fr/case-studies/hermes-agency-os/lessons-learned.md` | translated |  |
| `case-studies/hermes-agency-os/failure-modes.md` | `translations/fr/case-studies/hermes-agency-os/failure-modes.md` | translated |  |
| `case-studies/hermes-agency-os/autonomy-probe-pattern.md` | `translations/fr/case-studies/hermes-agency-os/autonomy-probe-pattern.md` | translated |  |
| `case-studies/hermes-agency-os/wrapper-and-checkpoint-pattern.md` | `translations/fr/case-studies/hermes-agency-os/wrapper-and-checkpoint-pattern.md` | translated |  |
| `audits/method-self-audit.md` | `translations/fr/audits/method-self-audit.md` | translated |  |
| `audits/risk-register.md` | `translations/fr/audits/risk-register.md` | translated |  |
| `audits/anti-patterns.md` | `translations/fr/audits/anti-patterns.md` | translated |  |
| `audits/alternatives.md` | `translations/fr/audits/alternatives.md` | translated |  |

## Notes

- `LICENSE`, `CODE_OF_CONDUCT.md`, `CONTRIBUTING.md`, `SECURITY.md` and `CHANGELOG.md` are not mirrored in this pass. The license text must remain legally unchanged; the security and contribution files can receive short French summaries later if maintainers want them.
- `TRANSLATION_POLICY.md` remains at repository root in English because it governs canonical source behavior for all contributors.
